...
type FooterNav = {
  ...
  footerResources: NavChild[];
  footerServices: NavChild[];
  footerIndustries: NavChild[];
};

export default function Footer({ nav, settings }: { nav: FooterNav; settings: Record<string, string> }) {
    ...
    <footer className="bg-brand-navy text-white">
      ...
      <div className="section-container py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="space-y-3 text-sm">
              ...
              {settings.contact_email && (
                <a href={`mailto:${settings.contact_email}`} className="flex items-center gap-3 text-white/60 hover:text-white transition-colors">
                  <Mail className="w-4 h-4 text-brand-red shrink-0" />
                  {settings.contact_email}
                </a>
              )}
              {settings.contact_phone && (
                <a href={`tel:${settings.contact_phone.replace(/\s/g, "")}`} className="flex items-center gap-3 text-white/60 hover:text-white transition-colors">
                  <Phone className="w-4 h-4 text-brand-red shrink-0" />
                  {settings.contact_phone}
                </a>
              )}
              {whatsappNumber && (
                <a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 text-white/60 hover:text-white transition-colors">
          ...
          {/* Links */}
          <FooterCol title="Solutions" links={nav.footerServices} />
          <FooterCol title="Industries" links={nav.footerIndustries} />
          <FooterCol title="Company" links={[...nav.footerCompany, ...nav.footerResources]} />
        </div>
      </div>
...
function FooterCol({ title, links }: { title: string; links: NavChild[] }) {
